...
석사과정에서 연구할 때에는 text mining 위주로 진행했기 때문에, lemmatization이라던가, stemming, stop word removing 등의 작업을 당연히 거쳤었다. 그러나 졸업 후 딥러닝을 공부하며 이러한 작업을 전혀 보지 못해 의아해 하고 있었는데, 최근에 회사 과제로 한국어 전처리에 대해 조사를 하며 잘못 알고 있던 오개념이나 추가 지식을 정리해볼까 한다.
...
html 태그를 제거하거나, 특수 문자, 기호 등을 제거한다.

## 3. 문장 단위 분절

입력 단위를 문장 단위로 바꾸어, 한 라인 당 한 문장이 있게끔 한다

## 4. 분절

형태소 분석/단순한 분절, 띄어쓰기 교정 등을 통해 정규화를 수행한다.

## 6. 서브워드 분절

BPE, WPM, Sentencepiece 등을 통해 단어를 더 작은 단어의 모음으로 쪼갠다.


# 형태소 분석과 subword segmentation은 같은 단위에서 이루어지나? (즉, tokenizing 개념인가?)
...
그러나 [핑퐁 블로그](https://blog.pingpong.us/dialog-bert-tokenizer/)를 보면, 공백 기반/형태서 분석기 기반/ BPE 기반 tokenizer로 구분하는 것을 볼 수 있다. 또한, [당근 마켓 블로그](https://medium.com/daangn/%EB%94%A5%EB%9F%AC%EB%8B%9D%EC%9C%BC%EB%A1%9C-%EB%8F%99%EB%84%A4%EC%83%9D%ED%99%9C-%EA%B2%8C%EC%8B%9C%EA%B8%80-%ED%95%84%ED%84%B0%EB%A7%81%ED%95%98%EA%B8%B0-263cfe4bc58d)를 보면 형태소 분석과 subword 분절 중 하나만 선택하는 늬앙스로 이야기 하고 있다.
...
# PLM외에 TF-IDF/Word embedding 같은거는 전혀 안쓰나?
